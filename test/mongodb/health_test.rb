...
class MongoHealthTest < MongoTestCase
  ...
  test "pool wait and exhaustion publish measured CMAP telemetry" do
    wait_events = capture_events("mongo_pool_checkout_wait.solid_queue") do
      with_exhausted_pool(wait_queue_timeout: 0.2) do |client, release|
  ...
  test "polling silence is local to its thread and restores the logger" do
    ...
    refute_includes output.string, "silenced polling log"
  ...
  private
    def assert_fork_safe_client(client: nil, mongoid_client: nil)
      ...
      refute_equal result.fetch(:inherited_id), result.fetch(:child_id)
      refute_match(/Detected PID change/, result.fetch(:driver_log))
