...
- **`:at_most_once`.** `perform` runs only after the ownership-guarded `start` records `started_at`, as for deduplicated and limited jobs. Death paths fail a started claim, and `ClaimedExecution#rerunnable?` keeps it out of death recovery.
